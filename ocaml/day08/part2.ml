  ...
  (let lines = Array.of_list (Util.read_lines "./day08/input") in
   ...
   let rec loop sorted_distances =
     ...
     | (a, b, _) :: rest ->
         ...
         in
         if
           Hashtbl.length circuits = Array.length lines
           && Hashtbl.length sets = 1
         then
           let _, xa, _, _ = List.nth points a in
           let _, xb, _, _ = List.nth points b in
           xa * xb
         else loop rest
   in
   let result = loop sorted_distances in
   (* let () =
     Printf.printf "%s\n\n"
       (String.concat "\n"
          (List.map
